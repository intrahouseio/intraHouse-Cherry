...
### intraHouse-Cherry -  v4.7.10 (23.03.21)
* Исправлена ошибка аргумента сценария, передающегося через кнопу на интерфейсе. 
 Ошибка была внесена в версии v4.7.8

### intraHouse-Cherry -  v4.7.8 (16.03.21)
#### Обновление связано с изменениями в протоколе p2p
Требуется обновление движка nodejs с 8.0 до 8.17.  
В связи с этим обновление выполняется путем установки системы поверх рабочей через curl:    
https://intrahouse.ru/knowledge-base/ih-cherry-install/ihsetup/.  
Также требуется обновление плагинов p2p, cctv

В основном функционале системы: 
* При нажатии кнопки в поле аргумент передается объект с контекстом (экран, контейнер, ...) 

### intraHouse-Cherry -  v4.7.7 (18.12.20)
* Изменена процедура архивирования проекта - исключены папки, содержащие объемные оперативные данные 
* Исправлена ошибка состояния сценария, имеющего функцию boot  

### intraHouse-Cherry - v4.7.6 (13.10.20)
* Перезапуск dbagent-а при ошибке БД (до 5 попыток)

### intraHouse-Cherry - v4.7.5 (30.06.20)
* Интервал для записи в БД может быть от 100 мсек (Период: 0.1) 
* Добавлена команда сценария для переключения параметров записи устройства: device.dbSaveSetting({dbtm:0.1, dbcalc_type: 'last'});) 

### intraHouse-Cherry - v4.7.4 (26.06.20)
* Добавлена команда для перезапуска плагина из сценария: this.pluginRestart('имя экземпляра плагина') 
* Исправлена ошибка: при большом количестве плагинов и частой их перезагрузке (вследствие потери связи, например) со временем плагины переставали автоматически перезагружаться (висели в стопе)

### intraHouse-Cherry - v4.7.3 (17.03.20)
* Решение проблемы с потерей ключа. Проблема может возникать при использовании нестандартных сетевых интерфейсов после hard reset. Добавлен параметр softreset.  

### intraHouse-Cherry - v4.7.2 (03.03.20) 
* Исправлена ошибка: при перезагрузке сервера не сохранялся флаг блокировки устройства
* Исправлена ошибка таймеров в сценарии. Ошибка возникала, если таймер неоднократно сбрасывался/взводился, при этом последний раз непосредственно перед завершением сценария
* Решение проблем с доступом по https
